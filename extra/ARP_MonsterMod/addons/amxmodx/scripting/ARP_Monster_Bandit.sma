...
new gModel[] = "models/player/merc/merc.mdl"
new gClassname[] = "monster_arp_bandit"

new gWeaponModel[] = "models/p_m82.mdl"
new gWeaponSound[] = "weapons/barrett/fire.wav"
//new gWeaponSprite

new gDeadBodyClassname[] = "monster_arp_bandit_body"
...
public ARP_Init()
{
	ARP_RegisterPlugin( "Monster Mod - Bandit", "1.0", "Hawk552", "Fills the maps with monsters to be killed" )
	
	new file = ARP_FileOpen( "monsters-bandit.ini", "r" )
	if ( !file )
	{
		log_amx( "Error opening file: monsters-bandit.ini" )
		return
	}
...
public EventThink( ent )
{
	...
	if ( is_user_alive( player ) )
	{
		...
		if ( curTime - lastShot > 2.0 )
		{
			static Float:target[3], endPos[3], hit
			
			angles[0] += random_float( -2.0, 2.0 )
			angles[1] += random_float( -2.0, 2.0 )
			angles[2] += random_float( -2.0, 2.0 )
			angles[0] *= -1
			...
			pev( pev( ent, pev_euser3 ), pev_origin, angles )
...
dontTarget:
	...
	if ( !player || !is_user_alive( player ) )
	{
		...
		if ( vector_length( target ) < 1.0 || vector_distance( entOrigin, target ) < 25.0 
			|| 	( vector_length( boundaries1 ) > 1.0 && vector_length( boundaries2 ) > 1.0 &&
