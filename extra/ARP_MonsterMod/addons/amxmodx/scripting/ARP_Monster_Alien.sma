...
new gModel[] = "models/ProjectMobius/npcs/Enewa-02.mdl"
new gClassname[] = "monster_arp_alien"

//new gWeaponModel[] = "models/p_desert.mdl"
new gWeaponSound[] = "weapons/electro5.wav"
//new gWeaponSprite

new gDeadBodyClassname[] = "monster_arp_alien_body"
...
public ARP_Init()
{
	ARP_RegisterPlugin( "Monster Mod - Alien", "1.0", "Hawk552", "Fills the maps with monsters to be killed" )
	
	new file = ARP_FileOpen( "monsters-alien.ini", "r" )
	if ( !file )
	{
		log_amx( "Error opening file: monsters-alien.ini" )
		return
	}
...
public EventThink( ent )
{
	...
	if ( is_user_alive( player ) )
	{
		...
		if ( curTime - lastShot > 0.5 )
		{
			static Float:target[3], endPos[3], hit
			
			angles[0] += random_float( -1.0, 1.0 )
			angles[1] += random_float( -1.0, 1.0 )
			angles[2] += random_float( -1.0, 1.0 )
			angles[0] *= -1
